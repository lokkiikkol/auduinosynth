...
#define SUSTAIN4            120
#define RELEASE4            121
#define AMPLITUDE2          124
#define AMPLITUDE3          125
#define WAVEFORM3           127
#define WAVEFORM4            69
...
int8_t wavetable[LUTsize] {};
int8_t wavetable2[LUTsize] {};                                 
const int8_t sintable[LUTsize] PROGMEM = { // already biased with +127 phase to lowest points for no clicks
...
const int8_t triangletable[LUTsize] PROGMEM = {
...
const int8_t sawtoothtable[LUTsize] PROGMEM = {
...
const int8_t squaretable[LUTsize] PROGMEM = {
...
const int8_t noisetable[LUTsize] PROGMEM = {
...
// allocate space for 47 settings
static uint8_t settings[47];
...
void loop() { // *** MAIN LOOP ***
  ...
  o1.phase_increment = midinotetophaseinc[note] * pitchfinal;
  o2.phase_increment = (midinotetophaseinc[note + (transpose3 - 63) + bitRead(lfodestination,6) * lfosig] + bitRead(lfodestination,0)*lfosig + detune2*4) * pitchfinal;
  syncPhaseInc = mapMidi(note) * pitchfinal;
  sync2PhaseInc = (mapMidi(note + (transpose2 - 63) + bitRead(lfodestination,1)*lfosig) + detune) * pitchfinal;

  grainPhaseInc  = mapPhaseInc(envelope + ((aftertouch*(aftertouchimpact - 63)) >> 6) + bitRead(lfodestination,2)*lfosig)  >> 1;
  grainDecay     = graindecay +  bitRead(lfodestination,4)*lfosig ;
  grain2PhaseInc = mapPhaseInc(envelope2 + ((aftertouch*(aftertouch2impact - 63)) >> 6) + bitRead(lfodestination,3)*lfosig) >> 1;
...
ISR(PWM_INTERRUPT)
{
 ...
 switch (waveform3) {
   case 0:
  outputvalue = (o1.amplitude  * pgm_read_byte(sintable+((o1.phase>>16)%LUTsize)))>>8;
  break;
  case 1:
  outputvalue = (o1.amplitude  * pgm_read_byte(squaretable+((o1.phase>>16)%LUTsize)))>>8;
  break;
  case 2:
  outputvalue = (o1.amplitude * pgm_read_byte(sawtoothtable+((o1.phase>>16)%LUTsize)))>>8;
  break;
  case 3:
  outputvalue = (o1.amplitude * pgm_read_byte(triangletable+((o1.phase>>16)%LUTsize)))>>8;
  break;
  case 4:
  outputvalue = (o1.amplitude *  pgm_read_byte(noisetable+((o1.phase>>16)%LUTsize)))>>8;
  break;
 }
  
  switch (waveform4) {
   case 0:
  outputvalue += (o2.amplitude *  pgm_read_byte(sintable+((o2.phase>>16)%LUTsize)))>>8;
  break;
  case 1:
  outputvalue += (o2.amplitude *  pgm_read_byte(squaretable+((o2.phase>>16)%LUTsize)))>>8;
  break;
  case 2:
  outputvalue += (o2.amplitude *  pgm_read_byte(sawtoothtable+((o2.phase>>16)%LUTsize)))>>8;
  break;
  case 3:
  outputvalue += (o2.amplitude *  pgm_read_byte(triangletable+((o2.phase>>16)%LUTsize)))>>8;
  break;
  case 4:
  outputvalue += (o2.amplitude *  pgm_read_byte(noisetable+((o2.phase>>16)%LUTsize)))>>8;
  break;
 }
